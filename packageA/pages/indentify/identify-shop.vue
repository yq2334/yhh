				...
				<view class="pannel-body">
					...
					<view class="upload">
						...
						<u-upload :fileList="fileList" @afterRead="afterRead" @delete="deletePic" name="6" multiple
							...
							<image src="/packageA/static/images/shop-i.png" mode="widthFix"
								style="width: 483rpx;height: 304rpx;margin: 0 auto;display: block;"></image>
						</u-upload>
						<view class="flex justify-center align-center">
							<u--image class="margin-right-xs" :showLoading="true" src="/static/images/camera.png"
								width="35rpx" height="31rpx"></u--image>
							<text class="upload-name">点击上传店内环境照</text>
						</view>
	...
	export default {
		...
		methods: {
			...
			submit() {
				...
				if (!this.imgBase64URL) {
					uni.$u.toast('请上传店内环境照');
					return;
				}
				// if (!this.imgBase64URL) {
				...
				uploadComIng({
					orderno: this.form.orderno,
					basecode_in: this.imgBase64URL,
					// basecode_title: this.imgBase64URL,
				}).then((res) => {
					if (res.code == '00') {

						this.$tab.navigateTo('/packageA/pages/indentify/indentify-step-3?orderno=' + this.form
